...
const getComponentName = (originalName) => {
  originalName = originalName
    .replace(/^\d+/, (digits) => `${toWords.convert(parseInt(digits, 10))}_`)
    .replace(/_$/, "");
  return originalName.length === 1
    ? originalName.toUpperCase()
    : fastCase.pascalize(originalName);
};
...
const generate = async () => {
  ...
  await execa("npx", ["tsc", "--project", "./tsconfig.json"], {
    stdio: "inherit",
  });
  ...
  for (const filepath of esmFiles) {
    await fs.move(
      ...
      path.join(
        ...
        path.basename(filepath).replace(".js", ".esm.js")
      ),
      {
        overwrite: true,
      }
  ...
  for (const icon of icons) {
    await fs.outputFile(
      path.join(buildDir, icon.name, "package.json"),
      pkgJSONBuilt(icon.name)
    );
  }
